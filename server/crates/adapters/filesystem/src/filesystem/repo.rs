...
use kernel::{
    ...
    shared::crypto::hash,
    FileLocation, FileMetadata, StorageTier,
};
use medium::application::ports::FileStorage;
use snafu::ensure;
use tokio::{
    fs,
...
impl FilesystemStorageAdapter {
    ...
    fn get_full_path(&self, location: &FileLocation) -> PathBuf {
        let base = match location.storage_tier {
            StorageTier::Permanent => &self.settings.base_path,
            StorageTier::Temporary => &self.settings.tmp_path,
            StorageTier::Cache => &self.settings.cache_path,
        };
