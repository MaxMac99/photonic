...
use derive_new::new;
use kernel::{
    app_error::ApplicationResult, error::format_domain_error_with_backtrace as format_domain_error,
    FileLocation, MediumId, MediumItemId, StorageTier, UserId,
};
use tracing::{debug, error, info, instrument};

use crate::{
    application::ports::{FileStorage, MediumRepository},
    domain::StoragePathService,
};
...
impl MoveToPermanentStorageHandler {
    ...
    pub async fn handle(&self, command: MoveToPermanentStorageCommand) -> ApplicationResult<()> {
        ...
        for op in &operations {
            ...
            if let Err(copy_err) = self.file_storage.copy_file(&op.src, &op.dest).await {
                ...
                for completed in &completed_copies {
                    if let Err(rollback_err) = self.file_storage.delete_file(&completed.dest).await
                    {
                        error!(
        ...
        if let Err(save_err) = self.medium_repository.save(&medium).await {
            ...
            for op in &operations {
                if let Err(rollback_err) = self.file_storage.delete_file(&op.dest).await {
                    error!(
                        item_id = %op.item_id,
                        error = %format_domain_error(&rollback_err),
                        "CRITICAL: Failed to delete copied file during rollback. Manual cleanup required"
