...
impl MetaInfo {
    pub fn from(data: &[u8]) -> Result<Self, MetaError> {
        let mimetype: Mime = infer::get(data)
            .ok_or(MetaError::NoMimeType)?
            .mime_type()
            .parse()
            .map_err(|_| MetaError::NoMimeType)?;

        let mut cursor = Cursor::new(data);
        let exifreader = Reader::new();
        let exif = exifreader.read_from_container(&mut cursor).map_err(|err| MetaError::from(err))?;
...
mod test {
    ...
    use crate::common::meta::error::MetaError::Io;
    use crate::common::meta::MetaInfo;
    use crate::common::meta::metainfo::exif_datetime_to_chrono;
