...
use common::error::{FileNotExistsSnafu, InvalidPathSnafu, ParseSnafu, Result};
...
impl Exiftool {
    pub async fn new() -> Result<Self> {
        ...
        let stdout = process.stdout.take().context(ParseSnafu {
            message: String::from("Could not connect to exif_service"),
        })?;
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                if let Some(stdin) = stdin {
                    debug!("Send stop to exif_service");
                    let _ = stdin
                        .write_all(format!("-stay_open\nfalse\n").as_bytes())
                        .await;
                    let _ = process.wait();
                }
                debug!("Stopped exif_service");
            });
        });
    }
}
