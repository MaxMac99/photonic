...
impl Exiftool {
    pub async fn new() -> Result<Self, ExifError> {
        ...
        let stdout = process.stdout.take().context(ParseSnafu {
            message: String::from("Could not connect to exiftool"),
        })?;
    ...
    pub async fn read_file<P>(
        ...
        with_binary: bool,
        with_groups: bool,
    ) -> Result<HashMap<String, Metadata>, ExifError>
    where
        P: AsRef<Path>,
    {
        let path = file
...
mod test {
    ...
    async fn exif_heic() {
        ...
        let _res = exiftool.read_file(target_path, false, true).await.unwrap();
    }
    ...
    async fn exif_dng() {
        ...
        let res = exiftool.read_file(target_path, false, true).await.unwrap();
