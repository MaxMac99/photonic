...
use crate::domain::{medium::camera::GpsCoordinates, metadata::events::MetadataExtractedEvent};
...
impl EventProcessor<MetadataExtractedEvent> for MediumMetadataEnrichmentListener {
    ...
    async fn process(&self, event: MetadataExtractedEvent) -> ApplicationResult<()> {
        ...
        let gps_coordinates = event.metadata.location.as_ref().and_then(|loc| {
        ...
        debug!(
            "Enriched medium metadata for medium_id={}",
            event.medium_id
        );
